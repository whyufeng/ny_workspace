...
export default function EnhancedTable(prop) {
  ...
  const handleDelete = (data) => {
    const remainRows = rows.filter((row) => !data.includes(row.id));
    const remainCoins = remainRows.map((obj) => obj.name);
    setCoinList(remainCoins);
  };

  return (
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        <EnhancedTableToolbar Selected={selected} HandleDelete={handleDelete} />
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="right">{row.high24h}</TableCell>
                    <TableCell align="right">{row.low24h}</TableCell>
                  </TableRow>
                );
              })}
              {emptyRows > 0 && (
